...
The analysis began with preparing and validating a dataset of **150,000+** UAE e-commerce transaction records to ensure reliability for fraud detection. The following initial data quality checks were performed to establish a clean and trustworthy analytical foundation:
...
The chart shows fraudulent transactions by month with a very sharp spike in **January (7,723 cases)** compared to all other months. After January, fraud volumes drop drastically and remain relatively low and stable throughout the rest of the year, fluctuating mostly between 200–900 cases per month.
...
The chart shows signup-to-purchase time (datediff) aggregated by acquisition source, split into legitimate and fraudulent transactions. SEO and Ads dominate total signup-to-purchase duration, together accounting for the majority of overall customer activity, while Direct traffic shows significantly lower total datediff, reflecting fewer users and faster conversions.
...
The chart shows transaction and fraud distribution across age groups, with the highest activity concentrated between ages 20–40. Transaction volume peaks in the 26–30 age group (~29.4K transactions), followed by 31–35 (~25.5K) and 21–25 (~26.7K), indicating that young-to-mid working-age users dominate platform usage.
...
Age should be used as a weighted risk factor, especially when combined with source, purchase value (<50), and fast signup-to-purchase behavior.
...
The chart shows the distribution of legitimate vs fraudulent transactions by gender. Male users account for a higher overall transaction volume (~79.9K transactions) compared to female users (~57.1K transactions), and therefore also show a higher absolute count of fraudulent transactions.

In percentage terms, fraud rates are fairly comparable across genders. Female users have a fraud rate of ~10.0% (5,717 frauds out of 57,102 transactions), while male users show a slightly higher fraud rate of ~10.6% (8,434 frauds out of 79,859 transactions). This indicates that fraud differences are driven more by volume than gender-specific behavior.
...
Gender alone is not a strong standalone fraud predictor.
...
Based on the patterns across time, value, source, age, gender, city, and signup-to-purchase behavior, the business should shift from broad, static controls to **targeted, behavior-based fraud prevention**. The strongest lever is **early-lifecycle control**: introduce stricter checks for purchases within the **first 24 hours of signup**, especially same-day (Day 0) transactions, which account for **~75–80% of total fraud**. This should be combined with **dynamic transaction limits for low-value purchases (<50)** and rate-limiting the number of orders immediately after signup. Since fraud spikes during **January and early-month periods**, temporary seasonal rules (step-up verification, CAPTCHA, velocity checks) should be activated during clearance sales and major campaigns. Additionally, re-optimizing **Ads and SEO traffic**—the highest fraud-contributing sources—by tightening audience targeting, excluding high-risk geographies, and applying source-specific risk scoring will significantly reduce fraud inflow without impacting Direct traffic, which is consistently lower risk.

From a segmentation standpoint, fraud prevention should be **risk-weighted rather than demographic-blocked**. While gender differences are largely volume-driven, the **31–35 age group** shows disproportionately high fraud and should trigger **additional identity or device verification** when combined with fast signup-to-purchase behavior and low purchase values. Geo-based rules should flag **non-UAE or cross-border shipping cities**, especially when billing, BIN country, and IP signals do not align. Implementing a **composite fraud risk score** (signup timing + purchase value + source + age + geo mismatch) and routing only high-risk transactions for friction (OTP, ID check) will lower fraud while protecting genuine customer experience.
