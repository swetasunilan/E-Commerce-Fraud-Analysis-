...
SELECT
     DATEDIFF(signup_time,purchase_time) AS day,
     COUNT(CASE WHEN class = '0' THEN 1 ELSE NULL END) 'LEGIT',
     COUNT(CASE WHEN class = '1' THEN 1 ELSE NULL END) 'FRAUD'
FROM data
GROUP BY DATEDIFF(signup_time,purchase_time)
ORDER BY day DESC;
...
WITH CTE AS(SELECT
    ...
    ELSE NULL END purchase_value_group,
    class
FROM data)
SELECT
...
-- Final test — recent signup to purchase (DIRECT source) fraud percent
 MONTH(signup_time) AS signup_month,
 source,
 COUNT(CASE WHEN class = '0' THEN 1 ELSE NULL END)'legit_transaction',
 COUNT(CASE WHEN class = '1' THEN 1 ELSE NULL END)'fraudulent_transaction',
 ROUND(
    SUM(CASE WHEN class = '1' THEN 1 ELSE 0 END) * 100.0 / COUNT(*),2) AS fraudulent_transaction_percent
FROM data
WHERE DATEDIFF(purchase_time, signup_time) < 1 
   AND source = 'DIRECT' 
   AND MONTH(signup_time) < 2
GROUP BY signup_month, source
